...
class ChargingBehaviorSpec extends UnitSpec with ChargingBehaviorTestData {
  "The ChargingBehavior" should {
    ...
    "should check if ev wants to charge" in {
      ChargingBehavior.doesEvWantToCharge(evChargingNeeded.copyWith(zero), seed) shouldBe true
      ChargingBehavior.doesEvWantToCharge(ev.copyWith(ev.getEStorage), seed) shouldBe false
    }
    ...
    "should give correct rations for chargingStations" in {
      ...
      x shouldBe Map(UUID.fromString("7537c0b6-3137-4e30-8a95-db1c0f9d9b81") -> 4.0)
    }
  }
}
