...
trait ChargingBehaviorTestData extends TripSimulationData {
  protected val zero: ComparableQuantity[Energy] = Quantities.getQuantity(0, PowerSystemUnits.KILOWATTHOUR)
  protected val half: ComparableQuantity[Energy] = Quantities.getQuantity(50, PowerSystemUnits.KILOWATTHOUR)
  ...
  val evChargingNeeded : ElectricVehicle = ev.copyWith(
    ...
    destinationCategoricalLocation = CategoricalLocationDictionary.SUPERMARKET.id,
  ...
  protected val currentlyAvailableChargingPoints : Map[UUID, Integer] = {
    chargingStations.map{chargingStations =>
      chargingStations.getUuid -> chargingStations.getChargingPoints.asInstanceOf[Integer]
    }.toMap
  }
  ...
  protected val maxDistance : ComparableQuantity[Length] = Quantities.getQuantity(5000, Units.METRE)
